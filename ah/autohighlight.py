...
class Autohighlight(object):
    ...
    def __init__(self, file):
        ...
        self.ColorDefinitions = self.getPredefinedColors(
        )  # Color definition table - maps color names(as strings) to Color objects
        self.OrderedColorMappings = [
        ]  # A list of color Mapping objects in textual order (contents of thir section of input)
    ...
    def getPredefinedColors(self):
        ...
        colorNames = [
            'Comment', 'Constant', 'String', 'VariableName', 'FunctionName',
            'Keyword', 'Type', 'None', 'Error'
        ]
        colors = {}
        ...
        for token in self.tokenizer:
            ...
            if token.text == ".":
                ...
                if stack[0].text in self.GlobalSymbolDict:
                    originalDef = self.GlobalSymbolDict[
                        stack[0].text].defining_token
                    raise Exception(
                        "Symbol %s redefined at %d,%d. Originally at %d,%d" %
    ...
    def parse_cst(self):
        ...
        for token in self.tokenizer:
            ...
            if token.text == ".":
                ...
                if stack[
                        0].text in self.GlobalSymbolDict:  # Redefined lexical sym or add a new production?
                    existingSymbol = self.GlobalSymbolDict[stack[0].text]
                    if existingSymbol.is_gla:
                        raise Exception(
                            "Lexical Symbol %s redefined at %d,%d. Originally at %d,%d"
                            % (stack[0].text, stack[0].line, stack[0].col,
                               existingSymbol.defining_token.line,
                               existingSymbol.defining_token.col))
                    existingSymbol.productions += [
                        Production(existingSymbol, production_elements)
                    ]
    ...
    def mkColor(self, name):
        ...
        for token in self.tokenizer:
            if token.text == ";":
                stack[0].assert_symbol_name
                if stack[0].text not in known_attrs:
                    raise Exception(
                        "%d:%d: Unknown color attribute %s" %
                ...
                stack += [token]
        raise Exception(
            "%d:%d: End-of-file reached while scanning color %s defined here."
        ...
        for name in self.tokenizer:
            ...
            if midpunct.text == "{":
                ...
                if color in self.ColorDefinitions:
                    raise Exception(
                        "%d:%d: Color %s has already been defined." %
                        (name.line, name.col, name.text))
                ...
                for token in self.tokenizer:
                    if token.text == ".":
                        ...
                        raise Exception(
                            "%d:%d: Color section ended while defining mapping for color %s"
                            % (name.line, name.col, name.text))
                    try:
                        stack += [self.GlobalSymbolDict[token.text]]
                    except:
                        raise Exception(
                            "%d:%d: Literal %s does not occur in the grammar" %
                            (token.line, token.col, token.text))

            elif midpunct.text == '}':
                raise Exception(
                    "%d:%d: Coloring section ended unexpectedly here." %
                    (token.line, token.col))
            else:
                raise Exception("%d:%d: Expected : or {, not %s" %
                                (midpunct.line, midpunct.col, midpunct.text))
        raise Exception(
            "%d:%d: Unexpected end-of-file while scanning color definition section beginning here."
    ...
    def check_color_scoping(self):
        ...
        for mapping in self.OrderedColorMappings:
            if mapping.token.text not in self.ColorDefinitions:
                raise Exception("%d:%d Color %s is never defined" %
